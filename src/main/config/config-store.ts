...
export interface AppConfig {
  ...
  // Model response inactivity timeout in MINUTES. If the model / connection
  // stays silent for this long during a turn, the run is aborted. Default 10.
  ...
  private normalizeConfig(rawConfig: Partial<AppConfig> | undefined): AppConfig {
    ...
    const result: AppConfig = {
      ...
      responseTimeoutMinutes:
        typeof raw.responseTimeoutMinutes === 'number' &&
        Number.isFinite(raw.responseTimeoutMinutes)
          ? Math.max(1, Math.min(60, Math.round(raw.responseTimeoutMinutes)))
          : defaultConfig.responseTimeoutMinutes,
