...
import { ElectronOAuthClientProvider } from './oauth-provider';
...
export interface MCPServerConfig {
  ...
  enabled: boolean;
  autoApproveTools?: boolean; // Auto-allow this server's tools without asking permission
}
...
/**
 * If an HTTP(S) MCP URL embeds credentials (https://user:pass@host/...),
 * convert them into a Basic Authorization header and strip them from the URL.
 * `fetch()` ignores credentials embedded in the URL, so this is required for
 * basic-auth-protected HTTP/SSE MCP servers. Mutates `url` in place; returns the
 * merged headers (or undefined when there are none).
 */
function applyEmbeddedBasicAuth(
  url: URL,
...
/** Whether a header map already carries an Authorization header (any casing). */
function headersHaveAuthorization(headers?: Record<string, string>): boolean {
...
export class MCPManager {
  ...
  async removeServer(serverId: string): Promise<void> {
    ...
    // Drop any stored OAuth credentials so a re-added server re-authenticates.
    oauthTokenStore.clear(serverId, 'all');
    await this.refreshTools();
  }
  ...
  private async connectServerInternal(config: MCPServerConfig): Promise<void> {
    ...
    if (config.type === 'stdio') {
      ...
      // Move any embedded URL credentials into a Basic auth header.
      const sseHeaders = applyEmbeddedBasicAuth(sseUrl, config.headers);
      const sseHasBasicAuth = headersHaveAuthorization(sseHeaders);
      // Attach an OAuth provider for servers previously authenticated via OAuth
      // (e.g. PC1). Plain/basic-auth servers are unaffected.
      const sseAuthProvider = this.maybeCreateBackgroundOAuthProvider(config, sseHasBasicAuth);
      ...
      // Move any embedded URL credentials into a Basic auth header.
      const httpHeaders = applyEmbeddedBasicAuth(httpUrl, config.headers);
      const httpHasBasicAuth = headersHaveAuthorization(httpHeaders);
      // Attach an OAuth provider for servers previously authenticated via OAuth
      // (e.g. PC1). Plain/basic-auth servers are unaffected.
      const httpAuthProvider = this.maybeCreateBackgroundOAuthProvider(config, httpHasBasicAuth);
  ...
  /**
   * Build a NON-INTERACTIVE OAuth provider for a normal/background connect.
   * Returns undefined for basic-auth servers and for servers that have never
   * been authenticated via OAuth (so plain/public MCP servers are unaffected
   * and won't trigger OAuth discovery on an unrelated 401).
   */
  private maybeCreateBackgroundOAuthProvider(
    config: MCPServerConfig,
    hasBasicAuth: boolean
  ): ElectronOAuthClientProvider | undefined {
    if (hasBasicAuth) {
      return undefined;
    }
    if (!oauthTokenStore.hasTokens(config.id) && !oauthTokenStore.getClientInformation(config.id)) {
      return undefined;
    }
    return new ElectronOAuthClientProvider({
      serverId: config.id,
      // Placeholder — never used in non-interactive mode (redirect throws instead).
  ...
  /**
   * Open the OAuth login page (e.g. PC1's APEX login + OTP) in an in-app modal
   * window, so the user never has to leave Open Cowork. A persistent session
   * partition keeps the login cookie so future re-auth can be silent.
   */
  private openAuthWindow(authUrl: URL): BrowserWindow {
    ...
    const authWindow = new BrowserWindow({
      ...
      webPreferences: {
        ...
        partition: 'persist:mcp-oauth',
      },
    });
    // Keep any popups (rare for APEX) inside the same window flow.
  ...
  /**
   * Run the interactive OAuth 2.1 authorization-code (+ PKCE) flow for an
   * HTTP/SSE MCP server, ONCE, at assignment time. Opens the login page in an
   * in-app modal window for the user to log in (and pass OTP), captures the
   * redirect on a loopback server, exchanges the code for tokens, persists them,
   * then establishes the real tracked connection. Subsequent connects reuse +
   * auto-refresh the token.
   */
  async authenticateServer(
    config: MCPServerConfig
  ): Promise<{ authenticated: boolean; alreadyAuthenticated: boolean }> {
    if (config.type !== 'sse' && config.type !== 'streamable-http') {
      throw new Error('OAuth authentication is only available for HTTP/SSE MCP servers');
    }
    if (!config.url) {
      throw new Error(`Server ${config.name} requires a URL`);
    }

    // Keep our in-memory config in sync so a subsequent connectServer works.
    this.serverConfigs.set(config.id, config);

    // Disconnect any existing live connection so we re-authenticate cleanly.
    ...
    const provider = new ElectronOAuthClientProvider({
      serverId: config.id,
      redirectUri: loopback.redirectUri,
      interactive: true,
      onRedirect: (authUrl) => {
        log(`[MCPManager] ${config.name}: opening in-app login window for ${authUrl.host}`);
        authWindow = this.openAuthWindow(authUrl);
        // If the user closes the window before finishing, abort the wait.
        authWindow.on('closed', () => {
          authWindow = null;
          loopback.cancel(new Error('Cửa sổ xác thực đã bị đóng trước khi hoàn tất.'));
        });
      },
    });

    const headers = applyEmbeddedBasicAuth(url, config.headers);
    const requestInit: RequestInit = {};
    if (headers) {
    ...
    const client = new Client({ name: 'ai-ierp', version: '0.1.0' }, { capabilities: {} });

    let alreadyAuthenticated = false;
    try {
      try {
        // If valid tokens already exist this succeeds with no login round-trip.
        await client.connect(transport);
        alreadyAuthenticated = true;
        log(`[MCPManager] ${config.name} already authenticated (valid token present)`);
      } catch (err) {
        if (!(err instanceof UnauthorizedError)) {
          // A real connection error (network/DNS/etc.), not an auth challenge.
          throw err;
        }
        // Expected path: the login window opened; wait for the loopback callback.
        log(`[MCPManager] ${config.name}: waiting for OAuth authorization code...`);
      }
      ...
      if (!alreadyAuthenticated) {
        ...
        log(`[MCPManager] ${config.name}: token exchange complete`);
      }
    } finally {
      if (authWindow && !(authWindow as BrowserWindow).isDestroyed()) {
        (authWindow as BrowserWindow).close();
      }
      loopback.close();
    ...
    // Establish the real, tracked connection using the freshly stored tokens.
    await this.connectServer(config);
    await this.refreshTools();
    return { authenticated: true, alreadyAuthenticated };
  }

  /** Whether a server has completed OAuth (has stored tokens). */
  getOAuthStatus(serverId: string): { authenticated: boolean } {
    return { authenticated: oauthTokenStore.hasTokens(serverId) };
  }

  /** Sign out: drop stored OAuth credentials and disconnect the server. */
  async clearOAuth(serverId: string): Promise<void> {
    oauthTokenStore.clear(serverId, 'all');
