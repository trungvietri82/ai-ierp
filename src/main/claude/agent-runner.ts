...
function buildMcpCustomTools(
  ...
  // Skip tools the user disabled via the MCP connectors screen so the model
  // never even sees them (claude.ai-style per-tool on/off).
  const disabled = new Set(mcpConfigStore.getDisabledTools());
  const mcpTools = mcpManager.getTools().filter((t) => {
    if (disabled.has(t.name.toLowerCase())) return false;
    // Per-chat MCP selection: drop tools from servers turned off for this session.
    if (disabledServerIds && disabledServerIds.has(t.serverId)) return false;
    return true;
  });
...
interface CachedPiSession {
  ...
  // Fingerprint of the MCP tool set the session was built with (per-chat
  // selection + available tools + globally disabled tools). When it changes
  // (e.g. the user toggles a connector mid-chat) the session is recreated so
  // the new tool filter takes effect — reused sessions keep their original tools.
...
${hints.join('\n')}
  ...
  private installPermissionHook(piSession: PiAgentSession, sessionId: string): void {
    ...
    // A tool is auto-approved when its MCP server is marked "auto-approve tools".
    const isToolAutoApproved = (name: string): boolean => {
      if (!name.startsWith('mcp__') || !mcpManager) {
        return false;
      }
      const tool = mcpManager.getTools().find((t) => t.name === name);
      if (!tool) {
        return false;
      }
      return mcpConfigStore
        .getServers()
        .some((s) => s.name === tool.serverName && s.autoApproveTools);
    };
    ...
    agent.setBeforeToolCall(
      ...
      async (ctx: any, signal?: AbortSignal): Promise<any> => {
        ...
        // Hard block tools the user disabled in MCP settings (covers cached
        // sessions where the tool was registered before being turned off).
        if (mcpConfigStore.isToolDisabled(toolName)) {
          return {
            block: true,
            reason: `Tool '${getDisplayName(toolName)}' is disabled in MCP settings.`,
          };
        }

        let decision = decidePermission(sessionId, toolName, input);
        // Trusted MCP server → skip the prompt (override the default 'ask').
        if (decision === 'ask' && isToolAutoApproved(toolName)) {
          decision = 'allow';
        }
        ...
        if (decision === 'ask') {
          ...
          if (result === 'allow_always') {
            ...
            // Persist across sessions / restarts (claude.ai-style always-allow).
            persistAlwaysAllow(toolName);
          }
        }
  ...
  async run(session: Session, prompt: string, existingMessages: Message[]): Promise<void> {
    ...
    try {
      ...
      // Fingerprint the MCP tool set for this turn so a cached session is
      // recreated when the per-chat selection (or available/disabled tools)
      // changes — otherwise toggling a connector mid-chat has no effect.
      const mcpToolSignature = this.mcpManager
        ? JSON.stringify({
            d: Array.from(new Set(session.disabledMcpServerIds ?? [])).sort(),
            t: this.mcpManager
              .getTools()
              .map((tool) => `${tool.serverId}:${tool.name}`)
              .sort(),
            g: mcpConfigStore.getDisabledTools().slice().sort(),
          })
        : '';
      ...
      if (cachedSession && cachedSession.mcpToolSignature !== mcpToolSignature) {
        logCtx(
          '[ClaudeAgentRunner] MCP tool set changed, recreating cached pi session:',
          session.id
        );
        try {
          cachedSession.session.dispose();
          ...
          logWarn(
            '[ClaudeAgentRunner] dispose error while recreating pi session for MCP:',
            disposeError
          );
        }
        this.piSessions.delete(session.id);
...
- Use WebSearch/WebFetch only when Chrome MCP is unavailable or the user explicitly asks for generic web search.
        ...
        `<planning>
For MULTI-STEP tasks — anything that needs several tool calls or queries, a multi-part analysis, building a report or dashboard, or a request with roughly 3+ distinct steps — your VERY FIRST action MUST be to call the todo-list / task-plan tool to create the plan: about 3-7 short, concrete, action-oriented steps. Do this BEFORE running any query, search, or write. Then execute the steps in order: before starting a step set it to in-progress, and the moment it finishes set it to completed — keep exactly ONE step in-progress at a time, and call the todo tool again to update status as you go. The plan + live status appear in the user's Progress panel.
Do NOT make a plan for simple single-step requests (a direct question, a quick lookup, a short explanation) — for those just answer directly per CHAT FIRST. The todo list IS the plan; never write the plan out as prose in the chat.
</planning>`,
        `<file_and_skill_handling>
Skills: A set of Skills is available to you (e.g. xlsx, docx, pptx, pdf). When a task matches a Skill, USE IT via the Skill tool instead of improvising your own approach.

Binary / Office files: NEVER use the "read" tool on binary documents (.xlsx, .xls, .docx, .doc, .pptx, .ppt, .pdf, images, archives) — reading their raw bytes yields garbage and can break the request. To inspect or process them:
- Spreadsheets (.xlsx / .xls / .csv): use the "xlsx" Skill, or run a script (e.g. python with openpyxl / pandas) via the bash tool.
- Word (.docx): use the "docx" Skill. PowerPoint (.pptx): use the "pptx" Skill. PDF (.pdf): use the "pdf" Skill.
The plain "read" tool is only for text files (code, .txt, .md, .json, .csv, .xml, etc.).

Never fabricate data: if you cannot actually read a file's real contents (or a tool fails), say so plainly and stop — do NOT invent numbers, rows, table values, or results.
...
Presenting output files: after you CREATE a file (HTML report, dashboard, document, image, ...), do NOT run a shell command to open it (no start / open / xdg-open commands). Instead, end your reply by referencing the file by its name/path on its own line (e.g. "doanh_thu_2026_360.html") — the app turns it into a clickable link that previews the file in-app. Let the user decide to open it.
      ...
      // Per-chat selection: exclude MCP servers turned off for this session.
      const disabledMcpServerIds = new Set(session.disabledMcpServerIds ?? []);
      const mcpCustomTools = this.mcpManager
        ? buildMcpCustomTools(this.mcpManager, disabledMcpServerIds)
        : [];
      ...
      // Throttle map for streaming tool-call argument updates (toolCallId -> last sent ms).
      // Lets the UI show file content (e.g. a write tool's HTML) typing out live
      // without flooding the renderer with an update per token.
      const toolDeltaLastSent = new Map<string, number>();
      ...
      // Activity-based timeout: reset the timer whenever the SDK sends events, so
      // this only fires if the model/connection goes SILENT for the whole window
      // (e.g. a stalled gateway). Configurable in API settings (1-60 min).
      const configuredTimeoutMin = configStore.get('responseTimeoutMinutes');
      const PROMPT_TIMEOUT_MS =
        (typeof configuredTimeoutMin === 'number' && configuredTimeoutMin > 0
          ? configuredTimeoutMin
          : 10) *
        60 *
        1000;
          ...
          switch (event.type) {
            case 'message_update': {
              ...
              if (ame.type === 'text_delta') {
                ...
                // Stream the tool's growing arguments (e.g. a write tool's file
                // content) so the UI can show the code typing out live. Throttled
                // to ~10 updates/sec to avoid UI thrash on large files.
                const toolContent = ame.partial?.content?.[ame.contentIndex];
                if (toolContent?.type === 'toolCall') {
                  const now = Date.now();
                  const last = toolDeltaLastSent.get(toolContent.id) ?? 0;
                  if (now - last >= 100) {
                    toolDeltaLastSent.set(toolContent.id, now);
                    this.sendToRenderer({
                      type: 'trace.update',
                      payload: {
                        sessionId: session.id,
                        stepId: toolContent.id,
                        updates: {
                          toolInput: (toolContent.arguments as Record<string, unknown>) || {},
                        },
                      },
                    });
                  }
                ...
                // Final flush so the indicator shows the complete content right
                // before the tool executes.
                const finalCall = ame.toolCall;
                if (finalCall?.id) {
                  toolDeltaLastSent.delete(finalCall.id);
                  this.sendToRenderer({
                    type: 'trace.update',
                    payload: {
                      sessionId: session.id,
                      stepId: finalCall.id,
                      updates: {
                        toolInput: (finalCall.arguments as Record<string, unknown>) || {},
                      },
                    },
                  });
                }
                ...
                if (!hasEmittedError) {
                  ...
                  this.sendMessage(session.id, {
                    ...
                    content: [
                      {
                        type: 'text',
                        text: `**Lỗi**: ${resolvedPayload.errorText}\n\n${
                          /\b4\d{2}\b/.test(resolvedPayload.errorText)
                            ? '_Vui lòng kiểm tra cấu hình rồi thử lại._'
                            : '_Hệ thống đang tự động thử lại, vui lòng đợi..._'
                        }`,
                      },
                    ],
                    timestamp: Date.now(),
      ...
      if (controller.signal.aborted && abortedByTimeout) {
        ...
        const errorMsg: Message = {
          ...
          content: [
            {
              type: 'text',
              text: '**Hết thời gian chờ**: Không nhận được phản hồi sau thời gian dài, thao tác đã bị huỷ.',
            },
          ],
          timestamp: Date.now(),
        };
