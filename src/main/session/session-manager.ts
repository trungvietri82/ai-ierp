...
/** Parse a JSON string-array column, returning [] on any malformed value. */
...
export class SessionManager {
  ...
  private createSession(
    ...
    return {
      ...
      // Seed from the global default chosen on the welcome screen, so the very
      // first message of a new chat already respects the MCP selection.
      disabledMcpServerIds: mcpConfigStore.getDefaultDisabledServers(),
      createdAt: now,
      updatedAt: now,
    };
  }
  ...
  private saveSession(session: Session) {
    this.db.sessions.create({
      ...
      disabled_mcp_servers: JSON.stringify(session.disabledMcpServerIds || []),
  ...
  private loadSession(sessionId: string): Session | null {
    ...
    return {
      ...
      disabledMcpServerIds: parseStringArray(row.disabled_mcp_servers),
  ...
  /**
   * Persist the per-chat MCP selection (the list of MCP server ids turned OFF
   * for this session). Applies to the next agent turn — the tool list is rebuilt
   * from the session on each run.
   */
  setSessionMcpSelection(sessionId: string, disabledServerIds: string[]): void {
    const normalized = Array.from(new Set(disabledServerIds.filter((id) => typeof id === 'string')));
    this.db.sessions.update(sessionId, {
      disabled_mcp_servers: JSON.stringify(normalized),
      updated_at: Date.now(),
    });
    log(
      `[SessionManager] Updated MCP selection for ${sessionId}: ${normalized.length} disabled`
    );
  }
