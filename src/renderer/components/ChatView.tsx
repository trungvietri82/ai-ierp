...
import { StreamingCodePreview } from './StreamingCodePreview';
import type { Message, ContentBlock } from '../types';
import { Send, Square, Loader2, Plug, X, Clock, ArrowDown } from 'lucide-react';
...
export function ChatView() {
  ...
  // The latest in-progress trace step → what the AI is doing right now
  // (e.g. running run_query, generating a file) so the indicator shows the
  // live activity instead of just a spinner + elapsed seconds.
  ...
  // While a content-writing tool (write, etc.) streams, surface its growing
  // `content` so the file types out live instead of just showing "running: write".
  const liveCode = useMemo(() => {
    const step = currentActivity;
    if (!step || step.type !== 'tool_call') return null;
  ...
  // Per-chat MCP selection: which connected servers are turned off for this chat.
  const disabledMcpSet = useMemo(
    () => new Set(activeSession?.disabledMcpServerIds ?? []),
    [activeSession?.disabledMcpServerIds]
  );
  const activeMcpCount = useMemo(
    () => activeConnectors.filter((c) => !disabledMcpSet.has(c.id)).length,
    [activeConnectors, disabledMcpSet]
  );
  const toggleMcpServer = useCallback(
    (serverId: string) => {
      if (!activeSessionId) return;
      const next = new Set(activeSession?.disabledMcpServerIds ?? []);
      if (next.has(serverId)) next.delete(serverId);
      else next.add(serverId);
      const disabledServerIds = Array.from(next);
      // Optimistic local update; persists + applies on the next turn.
      updateSession(activeSessionId, { disabledMcpServerIds: disabledServerIds });
      void window.electronAPI.mcp.setSessionSelection({
        sessionId: activeSessionId,
        disabledServerIds,
      });
    },
    [activeSessionId, activeSession?.disabledMcpServerIds, updateSession]
  );
  ...
  // Manual "scroll to latest" — forces a scroll even when the user is reading
  // older messages (scrollToBottom bails out in that case).
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-background">
          ...
          <>
            ...
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-mcp/8 border border-mcp/15 justify-self-end">
              ...
              <span className="text-xs text-mcp font-medium">
                {showConnectorLabel
                  ? t('chat.connectorCount', { count: activeMcpCount })
                  : activeMcpCount}
              </span>
            </div>
          </>
      ...
      <div ref={scrollContainerRef} className="flex-1 overflow-y-auto">
        ...
        >
          ...
          {/* Live activity indicator — shows the current tool/step (e.g. run_query)
              or "generating…" so a long turn never looks frozen. */}
          {hasActiveTurn && (
            <div>
              <div className="flex items-center gap-2.5 px-4 py-2.5 rounded-full bg-background/80 border border-border-subtle max-w-fit">
                <Loader2 className="w-4 h-4 text-accent animate-spin flex-shrink-0" />
                <span className="text-sm text-text-secondary truncate max-w-[55vw]">
                  {currentActivity
                    ? currentActivity.toolName
                      ? `${t('chat.running')}: ${currentActivity.title}`
                      : currentActivity.title
                    : partialMessage || partialThinking
                      ? t('chat.generating')
                      : t('chat.processing')}
                </span>
              </div>
              {/* Live code stream for content-writing tools */}
              {liveCode && <StreamingCodePreview content={liveCode.content} path={liveCode.path} />}
            </div>
          )}
      ...
      {/* Scroll-to-bottom button — floats above the input when scrolled up */}
      {showScrollToBottom && (
        <div className="relative">
          <button
            ...
            className="absolute left-1/2 -translate-x-1/2 -top-12 z-20 w-9 h-9 rounded-full bg-background border border-border shadow-md flex items-center justify-center text-text-secondary hover:text-text-primary hover:bg-surface-hover transition-colors"
          >
          ...
          >
            ...
            >
              <ComposerActionsMenu
                ...
                disabledIds={activeSession?.disabledMcpServerIds ?? []}
                onToggleServer={toggleMcpServer}
                onAttachFiles={handleFileSelect}
                onManageConnectors={() => {
              ...
              <div className="flex items-center gap-2">
                {/* Model selector */}
                <ModelPicker align="right" className="hidden sm:block" />
