...
export function WelcomeView() {
  ...
  // Per-chat MCP selection chosen here applies as the DEFAULT for the new chat.
  const mcpConnectors = useMcpConnectors(isElectron);
  const [defaultDisabledMcp, setDefaultDisabledMcp] = useState<string[]>([]);
  useEffect(() => {
    if (!isElectron) return;
    let cancelled = false;
    void (async () => {
      try {
        const r = await window.electronAPI.mcp.getDefaultSelection();
        if (!cancelled) setDefaultDisabledMcp(r.disabledServerIds || []);
      } catch {
        // ignore
      }
    })();
  ...
  const toggleDefaultMcpServer = (serverId: string) => {
    setDefaultDisabledMcp((prev) => {
      const next = new Set(prev);
      if (next.has(serverId)) next.delete(serverId);
      else next.add(serverId);
      const list = Array.from(next);
      void window.electronAPI.mcp.setDefaultSelection({ disabledServerIds: list });
      return list;
    });
  };
      ...
      <div className="max-w-[840px] w-full space-y-7 animate-fade-in">
        <div className="space-y-4 text-center">
          <div className="flex items-center justify-center gap-4">
            <img
              src={logoUrl}
              alt={t('welcome.logoAlt', { appName })}
              className="w-16 h-16 md:w-20 md:h-20 rounded-[1.4rem] object-contain border border-border-subtle bg-background/60 shadow-soft p-1"
            />
            ...
            <div className="flex items-center gap-3">
              ...
              {isElectron && (
                <ComposerActionsMenu
                  ...
                  disabledIds={defaultDisabledMcp}
                  onToggleServer={toggleDefaultMcpServer}
